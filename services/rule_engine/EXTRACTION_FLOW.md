...
| **复杂路径** | 正文页数 **大于** `EXTRACTION_SIMPLE_MAX_BODY_PAGES`，或 **`force_full_pipeline` 为 true** → 进入复杂侧路由；`needs_batching` 还会因 TOC、章节数、估算体量等置为 true；超过简单门闸后正文页数本身也会要求分批（见 `route_by_complexity`）。 |
...
| 变量 / 字段 | 默认 | 作用 |
|-------------|------|------|
| `EXTRACTION_SIMPLE_MAX_BODY_PAGES` | 10 | 简单路径门闸（正文页数 ≤ 该值且未强制全量 → 简单画像；**>** 该值则进入复杂侧并按 `VISION_BATCH_PAGES` 分批 vision）。 |
...
- **API 凭证与模型**：由 **`apps/web`** 在调用规则引擎时通过请求头 **`X-Boardrule-Ai-Config`**（v2，**`slots`**）传入（在 Web **`/models`** 配置 Gemini 或 OpenRouter）；规则引擎 `.env` 不再配置 `GOOGLE_API_KEY`。  
- **`DATABASE_URL`**：LangGraph **PostgresSaver** 必需。  
- **分流与分批**：`EXTRACTION_SIMPLE_MAX_BODY_PAGES`、`VISION_BATCH_PAGES`、`GEMINI_PRO_MAX_OUTPUT_TOKENS` 等：见 **§2.1**、`.env.example` 与 `README.md`。  
