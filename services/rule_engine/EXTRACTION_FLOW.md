...
| **复杂路径** | 正文页数 **大于** 上述阈值，或 **`force_full_pipeline` 为 true** → 进入复杂侧路由；`needs_batching` 由 TOC、章节数、估算体量、`EXTRACTION_COMPLEX_ROUTE_BODY_PAGES`（默认 15）等共同决定。 |
...
- **分流与分批**：`EXTRACTION_SIMPLE_MAX_BODY_PAGES`、`EXTRACTION_COMPLEX_ROUTE_BODY_PAGES`、`VISION_BATCH_PAGES`、`GEMINI_PRO_MAX_OUTPUT_TOKENS` 等：见 **§2.1**、`.env.example` 与 `README.md`。  
