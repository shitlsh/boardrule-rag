...
- 每日上限来自管理后台 **系统设置 → C 端对话限额**（每 IP、全站总量；`0` 表示不限制该项）
...
- **C 端对话限额**：**系统设置 → C 端对话限额** — 每个 IP 每日上限、**全站每日总量**（默认 1000；任一项填 0 表示该项不限制）。
- **微信小程序登录**：**系统设置 → 微信小程序** — 填写 AppID 和 AppSecret（AppSecret 加密存储，不明文保存）。
...
### ❌ 对话框出现「⏰ 今日提问次数已用完」或全站上限提示

**原因**：当前 IP 达到每 IP 上限，或全站 C 端总量达到上限。

**排查步骤**：
1. 打开 Next.js 管理后台 → **系统设置 → C 端对话限额**
2. 调高「每个 IP 每日」或「全站每日总量」，或将对应项设为 `0`（不限制）后保存
3. 若要立即清空计数，可在数据库中删除当日 `RateLimit` 行（如 `ip:…`、`global:c_end:…`）
